...
compile("zifpoisvalidation.cpp")
dyn.load(dynlib("zifpoisvalidation"))
...
width <- 4
height <- 4
plotdata<- data.frame(Y = Y)
pdf(file="Data.pdf",width=width,height=height) 

ggplot(plotdata,aes((1:length(Y)),Y))+
  geom_point(shape=1)+
  labs(x="Times",y=expression(y[i]))
  
dev.off()

## Plot residual against previous observation:
pdf(file="Resid.pdf",width=width,height=height)
...
## qqplot
pdf(file="zifpoisqq.pdf",width=width,height=height) 

...
Type objective_function<Type>::operator()()
{
  ...
  PARAMETER(transf_rho2);
  PARAMETER(transf_phi);
  ...
  for (int i = 0; i < stateDim; i++)
    for (int j = 0; j < stateDim; j++) {
      ...
      covObs(i, j) = pow(rho2, Type(abs(i - j))) * sdObs[i] * sdObs[j];
    }
  ...
  // state 1
  Type huge = 10;
  for (int i = 0; i < stateDim; i++) nll -= dnorm(u(i, 0), Type(0), huge, true);

  // states likelihood
  for (int i = 1; i < timeSteps; i++)
    nll += neg_log_density(u.col(i) - phi * u.col(i - 1));  
  // observation likelihooh
  for (int i = 0; i < timeSteps; i++) {
    nll += neg_log_densityObs(obs.col(i) - u.col(i),
                              keep.col(i));  
  }
  ADREPORT(rho);
  ADREPORT(rho2);
